...
void ili9431_init(void)
{
	...
	gpio_set_level(ILI9341_BCKL, ILI9341_BCKL_ON);
}
...
void disp_spi_init(void)
{
	...
	spi_bus_config_t buscfg = {
		...
		.sclk_io_num = ILI9341_CLK,
		.quadwp_io_num = -1,
		.quadhd_io_num = -1};

	spi_device_interface_config_t devcfg = {
		.clock_speed_hz = SPI_FREQ * 1000 * 1000, //Clock out at 80 MHz
		.mode = 0,								  //SPI mode 0
		.spics_io_num = ILI9341_CS,				  //CS pin
		.queue_size = 1,
		.pre_cb = NULL,
		.post_cb = NULL,
	};
	...
	ret = spi_bus_initialize(USE_SPI, &buscfg, 1);
	assert(ret == ESP_OK);

	//Attach the LCD to the SPI bus
	ret = spi_bus_add_device(USE_SPI, &devcfg, &spi);
	assert(ret == ESP_OK);
}
